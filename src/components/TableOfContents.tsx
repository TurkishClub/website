...
export function TableOfContents() {
  ...
  useEffect(() => {
    ...
    headings.forEach((heading, index) => {
        const text = heading.textContent || heading.innerText || `Section ${index + 1}`;
        const trimmedText = text.trim();
        
        // Only process headings that have IDs and haven't been seen before
        if (heading.id && !seenIds.has(heading.id) && !seenTitles.has(trimmedText)) {
  ...
  return (
    <div className="hidden xl:block w-full">
      <div className="bg-white rounded-lg shadow-lg p-4 border border-gray-200">
        <h3 className="font-semibold text-gray-900 mb-4 text-sm">İçindekiler</h3>
