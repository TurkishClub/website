...
const portableTextComponents = {
    block: {
        ...
        h1: ({ children }: any) => (
            <h1 className="text-5xl font-bold mt-20 mb-10 text-gray-900 border-b border-gray-200 pb-6">{children}</h1>
        ),
        h2: ({ children }: any) => {
            ...
            const slug = text.toLowerCase()
                .replace(/[^a-z0-9\u00C0-\u017F]/g, '-') // Include accented characters
                .replace(/-+/g, '-')
                .replace(/^-|-$/g, '') || 'section';
            
            // Add timestamp to ensure uniqueness
            const timestamp = Date.now().toString(36);
            const id = `section-${slug}-${timestamp}`;
            
            return (
                <h2 id={id} className="text-2xl font-semibold mt-12 mb-6 text-gray-800 scroll-mt-8">
                    {children}
                </h2>
            );
        },
        h3: ({ children }: any) => (
            <h3 className="text-xl font-medium mt-8 mb-4 text-gray-700">{children}</h3>
        ),
        h4: ({ children }: any) => (
            <h4 className="text-lg font-medium mt-6 mb-3 text-gray-700">{children}</h4>
        ),
    ...
    list: {
        bullet: ({ children }: any) => (
            <ul className="list-disc ml-6 mb-6 space-y-2 text-gray-700 text-lg">{children}</ul>
        ),
        number: ({ children }: any) => (
            <ol className="list-decimal ml-6 mb-6 space-y-2 text-gray-700 text-lg">{children}</ol>
        ),
    },
    // Style list items
    listItem: {
...
export default async function BlogPostPage(props: BlogPostPageProps) {
    ...
    // Process author image URL on server side
    const processedPost = {
        ...post,
        author: post.author ? {
            ...post.author,
            imageUrl: post.author.image ? urlFor(post.author.image).width(48).height(48).url() : undefined
        } : undefined
    }
